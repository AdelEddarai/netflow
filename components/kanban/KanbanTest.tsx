  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over.id) {
      setTasks((tasks) => {
        ...
        // Only move if the status is changing
    ...
    <div className='w-full mt-10 rounded-md'>
      <DndContext onDragEnd={handleDragEnd} collisionDetection={closestCenter}>
        <div className="flex space-x-4">
          {statuses.map(status => (
            <div key={status} className="flex-1 p-4 border bg-gray-900 rounded-md">
              <h2 className="text-lg font-bold mb-4">{status.charAt(0).toUpperCase() + status.slice(1)}</h2>
              <SortableContext items={tasks.filter(task => task.status === status).map(task => task.id)} strategy={horizontalListSortingStrategy}>
                <div className="h-64 p-2 rounded-md overflow-y-auto">
                  {tasks.filter(task => task.status === status).map((task, index) => (
                    <KanbanTask key={task.id} task={task} index={index} showSkeleton={showSkeleton} />
                  ))}
                </div>
              </SortableContext>
            </div>
...
const KanbanTask: React.FC<{ task: Task; index: number; showSkeleton: boolean }> = ({ task, index, showSkeleton }) => {
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className={`bg-black p-2 mb-2 shadow-md rounded-md cursor-pointer ${isDragging ? 'z-100' : ''}`}
    >
